...
class APITest(unittest.TestCase):
    ...
    def attributeRoundTrip(self, file_ending):
        ...
        if found_numpy:
            ...
            series.set_attribute(
                "nparr_csingle", np.array([1.2 - 0.3j, 2.3 + 4.2j], dtype=np.complex64)
            )
            series.set_attribute(
                "nparr_cdouble", np.array([4.5 + 1.1j, 6.7 - 2.2j], dtype=np.complex128)
            )
            if file_ending not in ["bp", "bp4", "bp5"]:
                series.set_attribute(
                    "nparr_clongdouble",
        ...
        if file_ending == "h5":
            ...
            try:
                ...
                self.assertEqual(
                    series.get_attribute("pystring3"), [c for c in "howdy, again!"]
                )
        else:
    ...
    def testOpenPMD_2_0(self):
        ...
        read_2_0 = io.Series("../samples/openpmd_2_0.json", io.Access.read_only)
        ...
        self.assertEqual(
            io.Unit_Dimension.as_maps(B.grid_unit_dimension),
        ...
        read_1_1 = io.Series("../samples/openpmd_1_1.json", io.Access.read_only)
    ...
    def test_memory_selection_write(self):
        ...
        Memory selections on write are supported by the ADIOS2 and HDF5
        backends.
        """
        if not found_numpy:
            return
    ...
    def makeIteratorRoundTrip(self, backend, file_ending):
        ...
        jsonConfig = {
            ...
            "adios2": {"engine": {"type": "bp4", "usesteps": True}},
        }
        series = io.Series(
        ...
        global_config = {
            "adios2": {
                "engine": {
                    "type": "bp3",
                    "unused": "parameter",
                    "parameters": {"BufferGrowthFactor": "2.0", "Profile": "On"},
                },
                "unused": "as well",
                "dataset": {
                    "operators": [
    ...
    def backend_keepalive_mesh_component(self, file_ending):
        ...
        component[:, :] = np.reshape(np.arange(100, dtype=np.dtype("float")), [10, 10])
        ...
        np.testing.assert_array_equal(
            loaded, np.reshape(np.arange(100, dtype=np.dtype("float")), [10, 10])
        )
    ...
    def backend_keepalive_particle_position(self, file_ending):
        ...
        def get_component_only():
            ...
            position.reset_dataset(io.Dataset(np.dtype("float"), [num_particles]))
        ...
        read = io.Series(path, io.Access.read_only)
        loaded = read.snapshots()[0].particles["electrons"]["position"]["x"][:]
        read.flush()
        np.testing.assert_array_equal(
            loaded, np.arange(num_particles, dtype=np.dtype("float"))
        )
