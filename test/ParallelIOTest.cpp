...
inline
void write_test(std::string file_ending)
{
    int mpi_s{-1};
    int mpi_r{-1};
    ...
    Series o = Series("../samples/parallel_write." + file_ending, AccessType::CREATE, MPI_COMM_WORLD);

    o.setAuthor("Parallel " + file_ending);
...
TEST_CASE( "hdf5_write_test_zero_extent", "[parallel]" )
{
    for ( auto const & p : backends )
    {
