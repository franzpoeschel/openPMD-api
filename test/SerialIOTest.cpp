...
void git_early_chunk_query(
    std::string const filename, std::string const species, int const step)
{
    try
    {
        Series s = Series(filename, Access::READ_ONLY);
...
TEST_CASE("git_adios2_early_chunk_query", "[serial][adios2]")
{
    git_early_chunk_query(
        "../samples/git-sample/3d-bp4/example-3d-bp4_%T.bp", "e", 600);
}
