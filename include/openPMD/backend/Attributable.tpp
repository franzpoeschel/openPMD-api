...
Attribute
AttributableImpl< AttributableWrapper >::getAttribute(
    std::string const& key) const
{
    auto it = get().m_attributes->find(key);
    if( it != get().m_attributes->cend() )
        return it->second;
...
template< typename AttributableWrapper >
bool
AttributableImpl< AttributableWrapper >::deleteAttribute(
    std::string const& key)
{
    if(Access::READ_ONLY == get().IOHandler->m_frontendAccess )
    ...
    if( it != get().m_attributes->end() )
    {
        ...
        aDelete.name = key;
        get().IOHandler->enqueue(IOTask(&get(), aDelete));
        get().IOHandler->flush();
        get().m_attributes->erase(it);
        return true;
    }
    return false;
}
...
AttributableImpl< AttributableWrapper >::attributes() const
{
    std::vector< std::string > ret;
    ret.reserve(get().m_attributes->size());
    for( auto const& entry : *get().m_attributes )
...
template< typename AttributableWrapper >
size_t
AttributableImpl< AttributableWrapper >::numAttributes() const
{
...
template< typename AttributableWrapper >
bool
AttributableImpl< AttributableWrapper >::containsAttribute(
    std::string const &key) const
{
    return get().m_attributes->find(key) != get().m_attributes->end();
}

template< typename AttributableWrapper >
std::string
AttributableImpl< AttributableWrapper >::comment() const
{
    return getAttribute("comment").template get< std::string >();
}

template< typename AttributableWrapper >
AttributableImpl< AttributableWrapper > &
AttributableImpl< AttributableWrapper >::setComment(std::string const& c)
{
...
template< typename AttributableWrapper >
void
AttributableImpl< AttributableWrapper >::flushAttributes()
{
    if( dirty() )
    {
        ...
        for( std::string const & att_name : attributes() )
        {
            ...
            get().IOHandler->enqueue(IOTask(&get(), aWrite));
        }
...
template< typename AttributableWrapper >
void
AttributableImpl< AttributableWrapper >::readAttributes()
{
    Parameter< Operation::LIST_ATTS > aList;
    get().IOHandler->enqueue(IOTask(&get(), aList));
    get().IOHandler->flush();
    ...
    for( auto const& att_name : tmpAttributes )
    {
        ...
        get().IOHandler->enqueue(IOTask(&get(), aRead));
        try
        {
            get().IOHandler->flush();
...
template< typename AttributableWrapper >
void
AttributableImpl< AttributableWrapper >::linkHierarchy(
    std::shared_ptr< Writable > const& w)
{
    auto handler = w->IOHandler;
    get().m_writable->IOHandler = handler;
    get().IOHandler = handler.get();
    auto writable = w.get();
    get().m_writable->parent = writable;
    get().parent = writable;
}
} // openPMD
