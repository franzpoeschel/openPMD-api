...
struct ClonableTrampoline
{
    ...
    py::handle get_python_handle() const
    {
    ...
    std::unique_ptr<ChildCpp> clone_impl() const
    {
        auto self = static_cast<ChildPy const *>(this);
        if (m_originalInstance)
        {
    ...
    , ClonableTrampoline<chunk_assignment::PartialStrategy, PyPartialStrategy>
{
    ...
    std::unique_ptr<PartialStrategy> clone() const override
    {
        return clone_impl();
    }
};
...
void init_Chunk(py::module &m)
{
    ...
    py::bind_vector<std::vector<ChunkInfo>>(m, "VectorChunkInfo");
