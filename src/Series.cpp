...
SeriesInterface::SeriesInterface(
    ...
    : AttributableInterface{attri}, m_series{series}
{}

std::string SeriesInterface::openPMD() const
{
    return getAttribute("openPMD").get<std::string>();
}

SeriesInterface &SeriesInterface::setOpenPMD(std::string const &o)
{
    setAttribute("openPMD", o);
    return *this;
}

uint32_t SeriesInterface::openPMDextension() const
{
    return getAttribute("openPMDextension").get<uint32_t>();
}

SeriesInterface &SeriesInterface::setOpenPMDextension(uint32_t oe)
{
    setAttribute("openPMDextension", oe);
    return *this;
}

std::string SeriesInterface::basePath() const
{
    return getAttribute("basePath").get<std::string>();
}

SeriesInterface &SeriesInterface::setBasePath(std::string const &bp)
{
...
std::string SeriesInterface::meshesPath() const
{
    return getAttribute("meshesPath").get<std::string>();
}

SeriesInterface &SeriesInterface::setMeshesPath(std::string const &mp)
{
    auto &series = get();
    if (std::any_of(
...
std::string SeriesInterface::particlesPath() const
{
    return getAttribute("particlesPath").get<std::string>();
}

SeriesInterface &SeriesInterface::setParticlesPath(std::string const &pp)
{
    auto &series = get();
    if (std::any_of(
            series.iterations.begin(),
...
std::string SeriesInterface::author() const
{
    return getAttribute("author").get<std::string>();
}

SeriesInterface &SeriesInterface::setAuthor(std::string const &a)
{
    setAttribute("author", a);
    return *this;
}

std::string SeriesInterface::software() const
{
    return getAttribute("software").get<std::string>();
}

SeriesInterface &SeriesInterface::setSoftware(
...
std::string SeriesInterface::softwareVersion() const
{
    return getAttribute("softwareVersion").get<std::string>();
}

SeriesInterface &SeriesInterface::setSoftwareVersion(std::string const &sv)
{
    setAttribute("softwareVersion", sv);
    return *this;
}

std::string SeriesInterface::date() const
{
    return getAttribute("date").get<std::string>();
}

SeriesInterface &SeriesInterface::setDate(std::string const &d)
{
    setAttribute("date", d);
    return *this;
}

std::string SeriesInterface::softwareDependencies() const
{
...
std::string SeriesInterface::machine() const
{
...
IterationEncoding SeriesInterface::iterationEncoding() const
{
    return get().m_iterationEncoding;
}

SeriesInterface &SeriesInterface::setIterationEncoding(IterationEncoding ie)
{
    auto &series = get();
    if (written())
        throw std::runtime_error(
...
std::string SeriesInterface::iterationFormat() const
{
    return getAttribute("iterationFormat").get<std::string>();
}

SeriesInterface &SeriesInterface::setIterationFormat(std::string const &i)
{
    if (written())
        throw std::runtime_error(
...
std::string SeriesInterface::name() const
{
    return get().m_name;
}

SeriesInterface &SeriesInterface::setName(std::string const &n)
{
    auto &series = get();
    if (written())
        throw std::runtime_error(
    ...
    if (series.m_iterationEncoding == IterationEncoding::fileBased &&
        !auxiliary::contains(series.m_name, "%T"))
        throw std::runtime_error(
            "For fileBased formats the iteration regex %T must be included in "
            "the file name");
...
std::string SeriesInterface::backend() const
{
    return IOHandler()->backendName();
}

void SeriesInterface::flush()
{
    auto &series = get();
    flush_impl(
        series.iterations.begin(),
...
SeriesInterface::parseInput(std::string filepath)
{
    std::unique_ptr<SeriesInterface::ParsedInput> input{
        new SeriesInterface::ParsedInput};
...
void SeriesInterface::initDefaults(IterationEncoding ie)
{
...
void SeriesInterface::flushMeshesPath()
{
...
void SeriesInterface::flushParticlesPath()
{
    Parameter<Operation::WRITE_ATT> aWrite;
...
void SeriesInterface::readFileBased()
{
    auto &series = get();
    ...
    if (series.iterations.empty())
    {
        /* Frontend access type might change during SeriesInterface::read() to
         * allow parameter modification. Backend access type stays unchanged for
         * the lifetime of a Series. */
...
void SeriesInterface::readGorVBased(bool do_init)
{
    auto &series = get();
    Parameter<Operation::OPEN_FILE> fOpen;
...
AdvanceStatus SeriesInterface::advance(
    AdvanceMode mode,
...
namespace
{
    template <typename T>
    void
    getJsonOption(nlohmann::json const &config, std::string const &key, T &dest)
    {
