...
SeriesImpl& SeriesImpl::setOpenPMD(std::string const& o)
{
    this->setAttribute("openPMD", o);
    return *this;
}

uint32_t SeriesImpl::openPMDextension() const
{
    return this->getAttribute("openPMDextension").template get<uint32_t>();
}

SeriesImpl& SeriesImpl::setOpenPMDextension(uint32_t oe)
{
...
std::string SeriesImpl::basePath() const
{
    return this->getAttribute("basePath").template get<std::string>();
}

SeriesImpl& SeriesImpl::setBasePath(std::string const& bp)
{
...
std::string SeriesImpl::meshesPath() const
{
    return this->getAttribute("meshesPath").template get<std::string>();
}

SeriesImpl& SeriesImpl::setMeshesPath(std::string const& mp)
{
    if(std::any_of(
...
std::string SeriesImpl::particlesPath() const
{
    return this->getAttribute("particlesPath").template get<std::string>();
}

SeriesImpl& SeriesImpl::setParticlesPath(std::string const& pp)
{
    if(std::any_of(
            get().iterations.begin(),
            get().iterations.end(),
...
std::string SeriesImpl::author() const
{
    return this->getAttribute("author").template get<std::string>();
}

SeriesImpl& SeriesImpl::setAuthor(std::string const& a)
{
    this->setAttribute("author", a);
    return *this;
}

std::string SeriesImpl::software() const
{
    return this->getAttribute("software").template get<std::string>();
}

SeriesImpl& SeriesImpl::setSoftware(
...
std::string SeriesImpl::softwareVersion() const
{
    return this->getAttribute("softwareVersion").template get<std::string>();
}

SeriesImpl& SeriesImpl::setSoftwareVersion(std::string const& sv)
{
    this->setAttribute("softwareVersion", sv);
    return *this;
}

std::string SeriesImpl::date() const
{
    return this->getAttribute("date").template get<std::string>();
}

SeriesImpl& SeriesImpl::setDate(std::string const& d)
{
    this->setAttribute("date", d);
    return *this;
}

std::string SeriesImpl::softwareDependencies() const
{
    return this->getAttribute("softwareDependencies").template get<std::string>();
}

SeriesImpl& SeriesImpl::setSoftwareDependencies(
...
std::string SeriesImpl::machine() const
{
    return this->getAttribute("machine").template get<std::string>();
}

SeriesImpl& SeriesImpl::setMachine(std::string const& newMachine)
{
    this->setAttribute("machine", newMachine);
    return *this;
}

IterationEncoding SeriesImpl::iterationEncoding() const
{
    return *get().m_iterationEncoding;
}

SeriesImpl& SeriesImpl::setIterationEncoding(IterationEncoding ie)
{
    if(this->written())
...
std::string SeriesImpl::backend() const
{
    return IOHandler()->backendName();
}

void SeriesImpl::flush()
{
    ...
    std::unique_ptr< SeriesImpl::ParsedInput > input )
{
    ...
    if(IOHandler()->m_frontendAccess == Access::READ_ONLY || IOHandler()->m_frontendAccess == Access::READ_WRITE )
    {
        ...
        auto newType = const_cast< Access* >(&writable()->IOHandler->m_frontendAccess);
    ...
    iterations_iterator begin, iterations_iterator end )
{
    ...
    else
    {
        if( !this->written() )
        {
            ...
            // @todo IOTask(this, ...)
            IOHandler()->enqueue(IOTask(writable(), fCreate));
        }
...
SeriesImpl::flushMeshesPath()
{
    ...
    IOHandler()->enqueue(IOTask(writable(), aWrite));
}
...
SeriesImpl::readFileBased( )
{
    ...
    for( auto const& entry : auxiliary::list_directory(IOHandler()->directory) )
    {
        ...
        if( isContained )
        {
            ...
            fOpen.name = entry;
            IOHandler()->enqueue(IOTask(writable, fOpen));
            IOHandler()->flush();
            ...
            IOHandler()->enqueue( IOTask( writable, aRead ) );
            IOHandler()->flush();
            ...
            read();
            IOHandler()->enqueue(IOTask(writable, fClose));
            IOHandler()->flush();
        }
    }
...
SeriesImpl::readBase()
{
    ...
    IOHandler()->enqueue(IOTask(writable, aList));
    IOHandler()->flush();
...
SeriesImpl::read()
{
    ...
    IOHandler()->enqueue(IOTask(&get().iterations, pOpen));
    ...
    IOHandler()->enqueue(IOTask(&get().iterations, pList));
    IOHandler()->flush();
...
typename SeriesImpl::iterations_iterator
