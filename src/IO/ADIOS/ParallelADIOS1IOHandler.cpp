...
ParallelADIOS1IOHandlerImpl::ParallelADIOS1IOHandlerImpl(
    AbstractIOHandler *handler, json::TracingJSON json, MPI_Comm comm)
    : Base_t{handler}, m_mpiInfo{MPI_INFO_NULL}
{
    int status = MPI_SUCCESS;
    status = MPI_Comm_dup(comm, &m_mpiComm);
    VERIFY(
        status == MPI_SUCCESS,
        "[ADIOS1] Internal error: Failed to duplicate MPI communicator");
...
std::future<void> ParallelADIOS1IOHandlerImpl::flush()
{
...
void ParallelADIOS1IOHandlerImpl::init()
{
    int status;
...
ParallelADIOS1IOHandler::ParallelADIOS1IOHandler(
    std::string path, Access at, json::TracingJSON json, MPI_Comm comm)
...
void ParallelADIOS1IOHandler::enqueue(IOTask const &i)
{
    switch (i.operation)
    {
...
int64_t ParallelADIOS1IOHandlerImpl::open_write(Writable *writable)
{
...
ADIOS_FILE *ParallelADIOS1IOHandlerImpl::open_read(std::string const &name)
{
    ADIOS_FILE *f;
    f = adios_read_open_file(name.c_str(), m_readMethod, m_mpiComm);
    VERIFY(
...
int64_t ParallelADIOS1IOHandlerImpl::initialize_group(std::string const &name)
{
    std::stringstream params;
    params << "num_aggregators="
           << getEnvNum("OPENPMD_ADIOS_NUM_AGGREGATORS", "1")
           << ";num_ost=" << getEnvNum("OPENPMD_ADIOS_NUM_OST", "0")
           << ";have_metadata_file="
           << getEnvNum("OPENPMD_ADIOS_HAVE_METADATA_FILE", "1")
           << ";verbose=2";
    ...
    status =
        adios_select_method(group, "MPI_AGGREGATE", params_str.c_str(), "");
    VERIFY(
        status == err_no_error,
...
#if openPMD_HAVE_MPI
ParallelADIOS1IOHandler::ParallelADIOS1IOHandler(
    std::string path, Access at, json::TracingJSON, MPI_Comm comm)
    : AbstractIOHandler(std::move(path), at, comm)
{
...
#else
ParallelADIOS1IOHandler::ParallelADIOS1IOHandler(
    std::string path, Access at, json::TracingJSON)
    : AbstractIOHandler(std::move(path), at)
{
    throw std::runtime_error(
        "openPMD-api built without parallel ADIOS1 support");
}
...
void ParallelADIOS1IOHandler::enqueue(IOTask const &)
{}
